...
alltext = np.concatenate(
    (data["FullDescription"].values, data["Title"].values))
...
for iter in range(0, 100):
    test_categories, train_categories, test_salaries, train_salaries = train_test_split(
        allTFIDF, salaries, random_state=iter)
    ...
    model.fit(
        train_categories[:, 0:train_categories.shape[1] / 2], train_salaries)
    ...
    model.fit(
        train_categories[:, train_categories.shape[1] / 2:], train_salaries)
    output = model.predict(test_categories[:, train_categories.shape[1] / 2:])
    justFullDescriptionFeaturesPerf[
        iter] = mean_absolute_error(test_salaries, output)
